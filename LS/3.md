...
When performing a command, such as ```run lern/ls/3``` or ```echo "Hello World!"```, parameters are split by spaces. Therefore, if you intend for one parameter to contain multiple words that are separated by spaces, the parameter must be quoted.
